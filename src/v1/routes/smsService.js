...
router.post("/register/:phoneNumber", async (req, res) => {
    ...
    for (let i=0; i< 6;i++){
        ...
        activationCode = activationCode + (number)
    }

    //add it to the users data so verification code could be confirmed later

    const url = "https://api.iletimerkezi.com/v1/send-sms/get/?username=5063060708&password=Pecm5835&text" + activationCode + "&receipents=" + phoneNumber + "&sender=APITEST"
